...
<body>
  ...
  <?php  if ($ip === true) {
    crear_editar_log($archivo, "El archivo " . __FILE__ . " ha sido cargado", 0, $_SERVER['REMOTE_ADDR'], ((array_key_exists("HTTP_REFERER", $_SERVER)) ? $_SERVER["HTTP_REFERER"] : "Sin referer"), $_SERVER["HTTP_USER_AGENT"]);
    ?>
    <div class="form-row">
      ...
      <div class="form-group col-md-5 ml-4 mr-4 justify-content-center align-self-center">
        ...
        <form name="frm_iniciar_sesion" id="frm_iniciar_sesion" action="core/process.php" method="post">
          ...
          <button type="submit" id="btn_ingresar" name="btn_ingresar" class="btn btn-primary mx-auto d-block"
            value="ingresar">Iniciar sesión</button>
        </form>
      </div>
    </div>
    <?php
  } else {    $redirect = "https://www.ufg.edu.sv/";
    echo ("Su dirección IP no tiene permitida la visita a esta página, será redirigido en breves segundos a un sitio seguro");
    crear_editar_log($archivo, "Dirección IP no autorizada ha cargado el archivo " . __FILE__ . " ha sido redirigido a: $redirect", 1, $_SERVER['REMOTE_ADDR'], ((array_key_exists("HTTP_REFERER", $_SERVER)) ? $_SERVER["HTTP_REFERER"] : "Sin referer"), $_SERVER["HTTP_USER_AGENT"]);
